...
/**
 * The {@code Color} class is used to encapsulate colors in the default sRGB
 ...
 * the color is completely transparent. When constructing a {@code Color} with
 * an explicit alpha or getting the color/alpha components of a {@code Color},
 ...
 * @see AlphaComposite
 */
public class MyColor implements Paint, java.io.Serializable {
     ...
     * components (no alpha). If {@code null} after object construction, this
     * must be an sRGB color constructed with 8-bit precision, so compute from
     * the {@code int} color value.
     *
     * @serial
    ...
    /**
     * The color value in the native {@code ColorSpace} as {@code float}
     * components (no alpha). If {@code null} after object construction, this
     * must be an sRGB color constructed with 8-bit precision, so compute from
     * the {@code int} color value.
     *
     ...
     * {@code null}, this is not valid data, so compute from the {@code int}
     * color value.
     *
     * @serial
     * @see #getRGBComponents
    ...
    /**
     * Checks the color {@code float} components supplied for validity. Throws
     * an {@code IllegalArgumentException} if the value is out of range.
     *
     * @param r the Red component
     * @param g the Green component
     * @param b the Blue component
     ...
     * {@code a} are outside of the range 0 to 255, inclusive
     * @param r the red component
     * @param g the green component
     * @param b the blue component
    ...
    /**
     * Creates a color in the specified {@code ColorSpace} with the color
     * components specified in the {@code float} array and the specified alpha.
     ...
     * {@code components} array or {@code alpha} is outside of the range 0.0 to
     * 1.0
     * @see #getComponents
     * @see #getColorComponents
     */
     ...
     * {@code brighter} and {@code darker} are inverse operations, the results
     * of a series of invocations of these two methods might be inconsistent
     ...
     * {@code null} and is a {@code Color} object that has the same red, green,
     * blue, and alpha values as this object.
     *
     ...
     * intended to be used only for debugging purposes. The content and format
     ...
     * {@code Color}. This method handles string formats that are used to
     ...
     * is then converted to a {@code Color} object.
     * <p>
     * If the specified property is not found or could not be parsed as an
     * integer then {@code null} is returned.
     *
     ...
     * integer which is then converted to a {@code Color} object.
     * <p>
     * If the specified property is not found or cannot be parsed as an integer
     * then the {@code Color} specified by the second argument is returned
     * instead.
     *
     * @param nm the name of the color property
     ...
     * <p>
     * If the specified property is not found or could not be parsed as an
     * integer then the integer value {@code v} is used instead, and is
     ...
     * floating-point values between zero and one (numbers in the range
     * 0.0-1.0). The {@code hue} component can be any floating-point number. The
     * floor of this number is subtracted from it to create a fraction between 0
     * and 1. This fractional number is then multiplied by 360 to produce the
     * hue angle in the HSB color model.
     * <p>
     * The integer that is returned by {@code HSBtoRGB} encodes the value of a
     * color in bits 0-23 of an integer value that is the same format used by
     * the method {@link #getRGB() getRGB}. This integer can be supplied as an
     * argument to the {@code Color} constructor that takes a single integer
     * argument.
     *
     ...
     * between zero and one (numbers in the range 0.0-1.0). The {@code h}
     ...
     * number is then multiplied by 360 to produce the hue angle in the HSB
     * color model.
     *
    ...
    /**
     * Returns a {@code float} array containing the color and alpha components
     * of the {@code Color}, as represented in the default sRGB color space. If
     * {@code compArray} is {@code null}, an array of length 4 is created for
     * the return value. Otherwise, {@code compArray} must have length 4 or
     ...
     * Otherwise, {@code compArray} must have length 3 or greater, and it is
    ...
    /**
     * Returns a {@code float} array containing the color and alpha components
     * of the {@code Color}, in the {@code ColorSpace} of the {@code Color}. If
     * {@code compArray} is {@code null}, an array with length equal to the
     * number of components in the associated {@code ColorSpace} plus one is
     * created for the return value. Otherwise, {@code compArray} must have at
     * least this length and it is filled in with the components and returned.
     *
     * @param compArray an array that this method fills with the color and alpha
     * components of this {@code Color} in its {@code ColorSpace} and returns
     ...
     * {@code compArray} is {@code null}, an array with length equal to the
     * number of components in the associated {@code ColorSpace} is created for
     * the return value. Otherwise, {@code compArray} must have at least this
    ...
    /**
     * Returns a {@code float} array containing the color and alpha components
     * of the {@code Color}, in the {@code ColorSpace} specified by the
     * {@code cspace} parameter. If {@code compArray} is {@code null}, an array
     * with length equal to the number of components in {@code cspace} plus one
     * is created for the return value. Otherwise, {@code compArray} must have
     * at least this length, and it is filled in with the components and
     * returned.
     *
     * @param cspace a specified {@code ColorSpace}
     * @param compArray an array that this method fills with the color and alpha
     * components of this {@code Color} in the specified {@code ColorSpace} and
     * returns
     * @return the color and alpha components in a {@code float} array.
     */
    ...
    /**
     * Returns a {@code float} array containing only the color components of the
     * {@code Color} in the {@code ColorSpace} specified by the {@code cspace}
     * parameter. If {@code compArray} is {@code null}, an array with length
     * equal to the number of components in {@code cspace} is created for the
     * return value. Otherwise, {@code compArray} must have at least this
     * length, and it is filled in with the components and returned.
     *
     * @param cspace a specified {@code ColorSpace}
     * @param compArray an array that this method fills with the color
     * components of this {@code Color} in the specified {@code ColorSpace}
     * @return the color components in a {@code float} array.
     */
